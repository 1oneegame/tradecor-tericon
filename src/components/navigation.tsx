        ...
        <div className="absolute left-1/2 transform -translate-x-1/2 flex flex-row gap-9 items-center justify-center border-2 border-gray-200 rounded-full bg-white px-4 py-1">
            {links.map((link) => (
              <div key={link.href} className="w-full h-full flex items-center justify-center">
                  <Link key={link.href} href={link.href} className={`text-blue-800 hover:text-blue-600 w-full h-full flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors duration-300 text-md px-4 py-1 ${isActive(link.href) && "border border-blue-800"}`}>
                  {link.label}
                  </Link>
              </div>
            ))}
